...
python -m src.pokeca.cli check-images              # カード画像が外から読めるか
```
...
| デッキの中身 (60枚) の取得 | ✅ **実機で検証済み**。全4150デッキが60枚ちょうど、名前の欠落0 |
| カードのテキストの取得 | ✅ **実機で検証済み**。2151/2151 取得済み |
| カード画像の直リンク | ✅ **実機で検証済み**。5/5 が `200 image/jpeg`。`check-images` で確認できる |
| 子ども向けページの「なかみ」 | ✅ **Chromium で実際に開いて検証済み**。24デッキ描画・JSエラー0・横スクロールなし |
